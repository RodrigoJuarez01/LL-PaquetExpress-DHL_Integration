...
var _TransferManager_instances, _TransferManager_uploadPart, _TransferManager_getObjectPart, _TransferManager_streamToBuffer;
...
class TransferManager {
    ...
    createMultipartInstance(key, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof options === 'string') {
                options = { uploadId: options };
            }
            if (!(options === null || options === void 0 ? void 0 : options.uploadId)) {
                options = {
                    uploadId: (yield this.bucket.initiateMultipartUpload(key, {
                        overwrite: options === null || options === void 0 ? void 0 : options.overwrite
                    })).upload_id
                };
            }
            return new multipart_upload_1.MultipartUpload(this.bucket, key, options === null || options === void 0 ? void 0 : options.uploadId);
        });
    }
    putObjectAsParts(key, body, partSize, options) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            yield (0, validator_1.wrapValidatorsWithPromise)(() => {
                (0, validator_1.isNonEmptyString)(key, 'key', true);
            ...
            let concurrency, overwrite = false;
            if (typeof options === 'number') {
                concurrency = options;
            }
            else {
                concurrency = (_a = options === null || options === void 0 ? void 0 : options.concurrency) !== null && _a !== void 0 ? _a : 5;
                overwrite = options === null || options === void 0 ? void 0 : options.overwrite;
            }
            // Initiate multipart upload session
            const initiateRes = yield this.createMultipartInstance(key, { overwrite });
            // Validate concurrency value
            if (concurrency < 1) {
                throw new error_1.CatalystStratusError('INVALID_CONCURRENCY', `Concurrency must be at least 1. Provided: ${concurrency}`);
            }
            ...
            return new Promise((resolve, reject) => {
                body.on('readable', () => __awaiter(this, void 0, void 0, function* () {
                    let chunk;
                    while ((chunk = body.read(chunkSize)) !== null) {
                        parts.push(__classPrivateFieldGet(this, _TransferManager_instances, "m", _TransferManager_uploadPart).call(this, initiateRes, chunk, partNumber++, chunk.length));
    ...
    getIterableObject(key, partSize, versionId) {
        return __asyncGenerator(this, arguments, function* getIterableObject_1() {
            ...
            this.validateObjectSize(Math.round(fileSize / (1024 * 1024)));
            let start = 0;
            while (start < fileSize) {
                const end = (Math.min(start + chunkSize, fileSize) - 1);
                const chunk = yield __await(__classPrivateFieldGet(this, _TransferManager_instances, "m", _TransferManager_getObjectPart).call(this, key, start, end, versionId));
                const buffer = yield __await(__classPrivateFieldGet(this, _TransferManager_instances, "m", _TransferManager_streamToBuffer).call(this, chunk));
    ...
    generatePartDownloaders(key, partSize, versionId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (start < fileSize) {
                const end = (Math.min(start + chunkSize, fileSize) - 1);
                ((startVal) => {
                    parts.push(() => __classPrivateFieldGet(this, _TransferManager_instances, "m", _TransferManager_getObjectPart).call(this, key, startVal, end, versionId));
                })(start);
                start = end + 1;
            }
            return parts;
    ...
    validateObjectSize(size) {
        if (size < 1) {
            throw new error_1.CatalystStratusError('INVALID_OBJECT_SIZE', `The file size (${size} MB) is too small for multipart upload. Use a putObject() for files smaller than 1 MB.`);
        }
        return true;
    }
    validatePartSize(partSize) {
        if (partSize < 1 || partSize > 100) {
            throw new error_1.CatalystStratusError('INVALID_PART_SIZE', `Invalid part size. Part size must be between 1 and 100 MB. Provided: ${partSize} MB`);
        }
        return true;
    }
}
...
_TransferManager_instances = new WeakSet(), _TransferManager_uploadPart = function _TransferManager_uploadPart(initiateRes, stream, partNumber, partSize) {
    return __awaiter(this, void 0, void 0, function* () {
        const res = yield initiateRes.uploadPart(stream, partNumber, { contentLength: partSize });
        if (res) {
...
}, _TransferManager_getObjectPart = function _TransferManager_getObjectPart(key, start, end, versionId, retries = 3) {
    return __awaiter(this, void 0, void 0, function* () {
        yield (0, validator_1.wrapValidatorsWithPromise)(() => {
            (0, validator_1.isNonEmptyString)(key, 'key', true);
        ...
        const res = yield this.bucket.getObject(key, { range: `${start}-${end}`, versionId });
        const err = new Promise((_resolve, reject) => {
            res.on('error', () => __awaiter(this, void 0, void 0, function* () {
                if (retries > 0) {
                    yield new Promise((resolve) => setTimeout(resolve, 1000));
                    yield __classPrivateFieldGet(this, _TransferManager_instances, "m", _TransferManager_getObjectPart).call(this, key, start, end, versionId, --retries);
                }
                else {
...
}, _TransferManager_streamToBuffer = function _TransferManager_streamToBuffer(stream) {
    return new Promise((resolve, reject) => {
        const chunks = [];
